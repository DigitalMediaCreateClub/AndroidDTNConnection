...
//			                			
			                			//·•ªƒf[ƒ^‚ª‚È‚¯‚ê‚Î‘—M‚µ‚È‚¢
			                			//‘ŠŽè‚ªŽ‚Á‚Ä‚¢‚È‚¢ƒnƒbƒVƒ…ƒŠƒXƒg‚É‘Î‚·‚éƒf[ƒ^‚ð‘—M
			                			if(hash.size() != 0){
			                				deviceName  = DTNMessageCollection.getDeviceNameListfromHash(hash);
				                			deviceIP    = DTNMessageCollection.getIPListfromHash(hash);
				                			chatMessage = DTNMessageCollection.getchatMessageListfromHash(hash);
				                			time.add("");
...
//						                				Log.d("Debug Max Message",_hash.get(k));
					                				...
					                				MessageInfo diffrenceMessageInfo = new MessageInfo(1, _deviceName, _deviceIP, _chatMessage, _time, _hash);
				                				...
				                				MessageInfo diffrenceMessageInfo = new MessageInfo(1, deviceName, deviceIP, chatMessage, time, hash);
					                			Send.sendByUDP(diffrenceMessageInfo);
				                			}
			                			}
				                	}
			                			...
			                			if(!Arrays.asList(messageInfo.hash).isEmpty()){
				                			...
				                			if(addData){
				                				for(int i = 1; i < hash.size(); i ++){
				                					...
				                					DTNMessageCollection.addData(deviceName.get(i), deviceIP.get(i), chatMessage.get(i), time.get(0), hash.get(i), "a", "a");
				                				}
				                			}
			                			}
			                		}
